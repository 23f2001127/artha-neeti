...
h1 { color: #12332c; font-size: 20pt; margin: 0 0 4px 0; }
...
p { margin: 4px 0; }
.meta { color: #5a5d55; font-size: 9pt; margin: 0 0 18px 0; }
...
def _company_report_html(ticker: str, report: dict) -> str:
    ...
    section_html = "".join(
        f"<h3>{_SPECIALIST_LABEL.get(key, key)}</h3><p>{_esc(text)}</p>"
        for key, text in sections.items()
        if text
    )
    return (
        f"<h2>{_esc(company)} ({_esc(ticker)})</h2>"
        f"<p>{_esc(report.get('executive_summary'))}</p>"
        f"{section_html}"
...
<body>
<h1>ArthaNeeti Research Report</h1>
<p class="meta">{_esc(query)}</p>
{body}
